/*
 *  arch/linux/aarch64/cpu_init.c
 *  Adapted from arch/aarch64/cpu_init.c. The only privileged-instruction
 *  reference (msr daifset in knl_cpu_shutdown) is replaced with a plain
 *  exit(0) since this is a hosted Linux process.
 */
