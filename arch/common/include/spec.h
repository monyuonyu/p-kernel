 ...
 *        してから学習 → 自然に別解へ収束する。
 *  ルーティングは spec 専用の領域ゲート (sp_band = 温度で定義される訓練バンド)
 *  を使い、入力の領域に対応する専門家を疎に発火させる (専門外は発火しない)。
 *  注 (wave 18): live な moe ゲートは学習 dtr の *クラス* 予測になったが、spec の
 *  ゲートは *バンド (領域)* 予測 — 別関数 (spec.c 冒頭 honest note 参照)。
 *
