/*
 *  test_all/test_task.c — タスク管理 テスト
 *
 *  テスト一覧:
 *    T10 — tk_ref_tsk:  タスク情報を参照 (優先度・ポリシー・スライス)
 *    T11 — tk_chg_pri:  実行中タスクの優先度を動的に変更
 *    T12 — tk_chg_slt:  タイムスライスを動的に変更
 *    T13 — tk_wup_tsk:  タイムアウト前にタスクを早期起床させる
 *    T14 — tk_slp_tsk:  タイムアウトで自然に起床 (E_TMOUT 確認)
 */
