/*
 *  05_all_demo/sched.c — スケジューリング デモ
 *
 *  デモ一覧:
 *    D15 — FIFO 優先度抢占: 高優先タスクが低優先タスクより先に実行される
 *    D16 — RR 同優先ローテーション: 同優先 RR タスクが交互に実行される
 *    D17 — FIFO + RR 混在: 同優先の FIFO/RR 混在キューが正しく動作する
 *    D18 — リソース再利用: タスクを 8 回繰り返し作成してリークがないことを確認
 *    D19 — タスク上限: USR_TASK_MAX (8) を超えると失敗することを確認
 */
