...
| **G28** | I19, I20, I21 | **【wave14 で接地+閉環】** ~~「脅威」が環境から接地されていない~~ → **threat の第二源を実体に接地し、それを縮める actuator を入れてループを閉じた。** (1) **守る対象を一級オブジェクト化**: `protect <ref>` で名前付き p-fs オブジェクトを「これは生き延びねばならない(守る UNIT)」と宣言する登録簿(`protect.c`)。(2) **接地した threat**: 宣言オブジェクトが `>=R` 近傍へ**まだ複製されていない(under-replicated)あいだだけ** threat=HIGH、複製が進むと DROP。threat = `(R−replicas)·STEP`(clamp; `protect_threat_for`)で、replicas は「保持を *announce してきた* 別ノード数」= **局所/ゴシップ状態のみ**(§7 中央 argmax なし)。world.c の `compute_threat()` が `max(reflex, protect)` を配り、近傍はこの一点へ rally する。(3) **actuator**: threat が立つあいだ `protect_tick` が対象を re-announce → 近傍が WANT→転送→**永続ストアへ複製** → 近傍が announce 返し → 所有ノードの replica カウントが上がり **threat がタイマでなく実在の複製で落ちる**。complete=replication が episode を終わらせる(`conserve_until` ではない)。**実証**: `protect test`(`[protect-ground]`/`[protect-loop]` PASS)+ `samples/27_protect`(threat 40→20→0 が複製2件の確認で起き、kill -9 後にオブジェクトが近傍の永続ストアから served; actuator OFF の対照では threat=40 据え置き・kill で LOST)。**【fix: w14 A2隊 — LATEST_ONLY 計数バグ】** 初版の live 3-node demo は **self-test は緑なのに本番で失敗**していた(G32 の生きた実例): announce 制御トピックが `KDDS_QOS_LATEST_ONLY`(depth-1, last-writer-wins; `pfs_repl.c:557-559`)のため、近傍2件が actuator の再announce に対し**同時に announce 返し**すると 1 スロットを奪い合い、owner がポーリングする前に上書きされる。結果、両近傍が実際に **store+announce 返し済み**(ログに `replicated block ... origin=n0` と `announce ... fanout=2`)なのに owner は永遠に `replicas=1/2`(threat 40→20、SAFE 不達)。kill 生存自体は機能していた(複製は届いていた)ので、壊れていたのは **R への計数収束のみ**。**修正**(`pfs_repl.c` ANN RX): 既に保持するブロックについて **origin 自身が再駆動した announce(`a.src_node == a.origin` = protect actuator の再announce のみ)** を聞いたら保持者は announce 返しする(gossip 自己修復)。holder↔holder メッセージ(返しは origin≠self を載せる)では発火させないので storm 化せず、per-(src,seq) dedup で actuator 周期あたり最大1返し。actuator の各周期が全保持者から **新 seq の返し**を誘発し、LATEST_ONLY の per-tick 取りこぼしを跨いで owner の `holder_count` が真値=R へ収束(threat 40→20→0)。CI に **live 経路**を `protect-loop-live` job として配線(下記/audit-5 G32)。6/6 連続 PASS。**【fix2: w14 A3隊 — 残り2つの flake を接地して 5/5 安定化】** A2隊の修正後も live demo は ~1/5 で落ちていた。実ログで2因を確定し両方修正:**(1) target_r が宣言タイミングで揺れる** — `cap_target_r()` は `region_size()-1` で R を抑えるが、`region_size()` は各 peer の RTT 実測(≤τ)を要し、degrade の `-> FULL`(ALIVE 数)より遅れる。FULL 直後に宣言すると region_size=2 → target_r=1 に抑えられ 1 複製で SAFE 到達、demo の「≥2 distinct neighbours」と食い違って FAIL。**修正**:① demo 側 — `cluster_up` が node1 の region が `size=3` になるまで待ってから宣言(target_r→2 確定)。② kernel 側(堅牢化)— `requested_r` を保存し、宣言後に region が育ったら `protect_tick` で target_r を**要求 R へ向け単調 up に再cap**(早すぎる宣言を自己修復; peer 離脱では下げない)。**(2) 対照(actuator OFF)が boot SYNC で守れてしまう** — 起動時 SYNC(`pfs_repl.c` 「全部くれ」)で近傍が node1 の保持ブロック(静かに put した守る対象も)を pull し、actuator OFF でも複製され kill 後に生存し得た = 対照が**偶然**で崩れる。**修正(option (a): 意図的拡散ルール)**:**actuator にまだ駆動されていない**守る対象は SYNC ストリームから**除外**する(`pfs_repl_set_sync_filter` フック → protect 側 `protect_sync_exclude`、`driven` フラグ)。§2「守る *力* が意図して複製する。ambient sync で漏れない」。通常ブロック・駆動済みの守る対象は従来通り SYNC 配信(P1/boot-sync の回帰なし; `11_distributed`/`13_survival_loop` で確認)。actuator は announce→WANT→転送で複製するので treatment は無影響。これで treatment(複製で SAFE・kill 生存)と control(AT-RISK 据え置き・kill で LOST)が**両方とも決定的に真**。5/5 連続 PASS。**残る正直な未解決**: ① **reflex 側の threat 源(温度バケツ `gate_predict`)は依然フィードフォワード** — 接地したのは protect 軸のみで、温度バケツを除去したわけではない(`max` で併存)。② 保持者の **liveness 未追跡** — 複製先が死んでも `holder_count` は減らず、threat が再上昇して再複製しない(再励起は未実装)。③ 守る対象の複製は意図的に **actuator 専管**(protect put は ambient P1 announce を抑止、かつ未駆動の守る対象は SYNC 除外)= 対照を正直にするための設計選択(option (a))で、現実系なら ambient 複製も併用したい。 | 実装: `protect.c`, `protect.h`, `world.c:compute_threat`(max), `pfs_repl.c`(announce-hook + reannounce + suppress + sync-filter)。実証: `samples/27_protect/run.sh`, `protect test`。残課題: `protect.c`(holder liveness 未追跡; 温度バケツは `moe.c:107-114` のまま) | 🟢 (部分: protect 軸は接地+閉環; 温度バケツ源と holder liveness は未) | ✅(コード/デモ) |
