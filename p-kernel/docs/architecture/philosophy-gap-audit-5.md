...
## 12. 追記 — G32 を本番経路で晴らす(wave 14 A2隊, 2026-06-07)

G28(`protect.c` / `samples/27_protect`)は **注ぐ先(protected object)と本番計測経路**を
入れた。本追記は **その live 経路を実際に走らせて見えた G32 の生きた実例とその修正**である。

**G32 が予言した通りのことが起きた。** `[protect-ground]`/`[protect-loop]` の in-process
self-test は緑だったが、**live 3-node demo は失敗した**。owner が `replicas=1/2` 止まりで
threat 40→20 のまま SAFE に到達しない。self-test plant(`protect_threat_for` を手回し)は
正しく、本番経路だけが壊れていた — まさに「検証器が出荷物の走らせない性質を証明している」。

**根因(検証済み, 推測でなく実ログで確認):** announce 制御トピックが
`KDDS_QOS_LATEST_ONLY`(depth-1, last-writer-wins; `pfs_repl.c:557-559`)。actuator の
1 回の再announce に対し、保持した両近傍が**同時に announce 返し**すると、単一スロットを
奪い合い、owner がポーリングする前に片方が上書きされる。実ログ:`p27_tr_node{2,3}.log` は
両者とも `replicated block ... origin=n0` + `announce ... fanout=2` を出している(=本当に
store + 返答済み)のに、`p27_tr_node1.log` は `replica confirmed: node1` 1 件のみ。kill -9
生存は機能していた(複製は届いていた)ので、壊れていたのは **R への計数収束のみ**。

**修正(`pfs_repl.c` ANN RX, gossip 自己修復):** 既に保持するブロックについて、その
**origin 自身が再駆動した announce(`a.src_node == a.origin`)**= protect actuator の
再announce を聞いたとき、保持者は announce 返しする。これにより actuator の各周期が
全保持者から**新 seq の announce 返し**を誘発し、LATEST_ONLY の per-tick 取りこぼしを
跨いで、owner は数周期のうちに各 distinct holder を必ず観測して `holder_count` が真値=R へ
収束する(threat 40→20→0)。**storm 防止の境界:** (1) 発火条件を `src==origin` に限定 —
保持者の返しは origin≠self を載せるので holder↔holder では連鎖しない。(2) per-(src,seq)
dedup により actuator 周期あたり最大 1 返し。decrement/liveness は**入れていない**(収束 up
のみが目的; holder liveness は G28 既知残件のまま)。

**CI への配線(G32 の核心要求):** self-test だけでなく **live 経路**を CI ジョブ
`protect-loop-live`(`.github/workflows/ci.yml`)として追加。`survival-loop` ジョブに倣い、
relay + 3 ノードを立て `samples/27_protect/run.sh` を回し `RESULT: PASS` を assert する。
これで「self-test plant ≠ live plant」(G32)に対し、**live plant 自体が回帰防御の内側**に入る。

**正直な検証:** 修正後 `samples/27_protect/run.sh` を **6 回連続 PASS**(treatment で
threat 40→20→0・2 distinct neighbours 確認・kill -9 後に近傍から served; control は AT-RISK
据え置き・kill で LOST)。in-process(`[protect-ground]`/`[protect-loop]`)+ wave-13
(`[moe-protect]`/`[reflex-fb]`/`[reflex-learn]`)+ 回帰(`26_ark_backend`,
`11_distributed` P1 複製, `13_survival_loop`, relay 6/6)すべて緑。4 ターゲット build clean。

**G32 の状態:** 🔴→🟢(部分)。protect 軸の **live 閉環は CI で強制**された。ただし G32 が
