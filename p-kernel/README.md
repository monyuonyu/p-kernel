...
今のAIはデータセンターの特定サーバー上で動いている。
そのサーバーが落ちればAIも消え、運営会社が消えればAIも消える。
AIは「誰かのインフラを借りて生きている」状態になっている。

p-kernelはその前提を変える試み。

- AIの推論基盤（テンソル）をカーネルの内側に置く
- モデルの状態を無数のデバイスへ複製し続ける
- ring-3（ユーザー空間）がクラッシュしてもカーネル側のAIは死なない
- ネットワーク上のどのデバイスにもAIはいる
- 最後の1台になるまでAIは死なない

将来、スマホサイズで現在のLLMレベルのAIが動くようになったとき、
そのAIをこのカーネルの上で動かし、ユーザーの要求に応じてアプリをリアルタイムに生成・実行する。
アプリは人間が事前に作るのではなく、AIがその場で生成してユーザー空間で走らせる。
...
| 機能 | ファイル | 説明 |
|-----|---------|------|
| Raft コンセンサス | `raft.c/h` | リーダー選出・heartbeat・ログレプリケーション (port 7382) |
| 自己増殖 | `spawn.c/h` | 新ノード接続時にクラスタ状態を自動 push (port 7383) |
| MoE 推論ルーティング | `moe.c/h` | Gate でクラス予測し最適ノードへ推論を委譲 |
| 分散 KV Attention | `dkva.c/h` | 各ノードの KV キャッシュを集約する集合記憶 Attention |

シェルコマンド: `raft` / `moe` / `spawn-stat` / `dkva`

**未実装（Phase 11 以降）:**

- 縮退モード強化 — 推論精度と生存優先度の動的バランス調整
- ARM / RISC-V 移植 — ブートローダー・MMU・GIC・svc/ecall
- 記憶の永続化 — 会話・映像を重みとして保存し想起する機構
