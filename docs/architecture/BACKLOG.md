...
| B1 | the unified survival loop — **correction 2026-09-21: L0 (STATE bus) and L1 (STATE-aware support routing + §8 hysteresis) are ALREADY SHIPPED + CI-gated** (`world.c`'s `world_self_state_step`/`moe.c`'s `moe_state_fold`, `survival l0\|l1` shell verbs, `tests/host/run_survival_l{0,1}.sh`, `ci.yml`'s `[survival-l0/l1] ALL PASS` jobs; both crown-preserving per §9/§10, commander-decided 2026-06-28). **Next is L2 (HIBERNATING), not L0** — stop before L4 (democratic retirement) | `20-architecture/survival-loop.md` §6-L2, §9, §10 |
