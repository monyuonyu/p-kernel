...
| B3 | self-access R1 (first T1 affordance `self_access_publish` shipped+audited 2026-09-21, `feat/self-access-r1-publish`; crown grows +1560B x86/+1440B aarch64 bare-metal (Berkeley `size` text; `.text` itself +644/+728) — **re-bless DONE 2026-09-24** (approved by mk_pino, `docs/audit-trail.md` CROWN RE-BLESS entry: aarch64 `dea715dd…` / x86 `1c8e6d16…`, only R1 moves the crown in `origin/master..master`; hashes independently re-measured by a later run 2026-09-24, all match). **Audit independently re-verified and merged to local master 2026-09-21** — see `docs/audit-trail.md`. **2026-09-23: `[self-access-r1-test]` wired into CI** — never had a CI assertion until now, run as its own isolated `net`+`body test` invocation) | `30-module/self-access-design.md` R1 |
...
| D5 | **2026-09-23: 14 more cert scripts found with ZERO CI wiring** (a cross-reference of every `tests/**/run_*.sh`/`*_cert.sh` against `ci.yml`, after this same run fixed 6 similar gaps — `[sign-genome]`/`[sign-selflayer-live]`/`[unbounded-dkva]`/L2+mind-pause+dmn-pause/federation-f1/self-access-r1, all now wired). **5 of the 7 cheap host-only ones now wired** (`run_mc0.sh`/`run_mc1.sh`/`run_diverse_proof.sh`/`run_ss1.sh`/`run_ss3.sh`, each verified locally then merged into `shipped-llm-certs`). **Two genuine open findings surfaced while checking the other 2, NOT wired:** (1) `run_ss2.sh` FAILS today against its own documented pre-SS-2 baseline (`5edbeeda`) — M-tier forward hash `0x2caed71d...`/loss `5.5598` (current) vs `0xb94b1d57...`/loss `5.5406` (base), a real, reproducible mismatch (verified with the correct base student.c+student.h supplied via `SS2_BASE_STUDENT`, ruling out a tooling artifact). (2) `run_ss3_blob.sh`: 11/12 checks PASS but `[ss3-blob-merge]` "merged held-out loss <= the worse parent (convergence)" FAILS, reproducibly (2 independent runs). Unknown for both whether this is (a) a genuine regression introduced somewhere unrelated to their own SS-2/SS-3 waves, or (b) baselines/assumptions too stale to mean anything after everything that has landed since — **needs investigation before deciding whether to fix code or retire/rebase either cert**. **Costlier (aarch64, needs `qemu-system-aarch64 -smp N`, some run 3-10x for flake-detection — budget/timeout judgment before wiring):** `tests/aarch64/run_mc2_boot.sh`/`run_mc2_equiv.sh`/`run_mc2_slice.sh` (slice is cheapest, no `-smp` needed), `run_smp1.sh` through `run_smp5.sh` (smp4's "one-mind under SMP" crown cert is probably the highest-value one of the 5). One branch per script (or small cluster), verify locally before wiring, same `timeout N | tee log.log` + `grep PASS` shape. **2026-09-23 follow-up — both findings root-caused, by bisecting `5edbeeda..HEAD` commit-by-commit for each:** both first diverge at the exact same commit, `7d497d7c` (SCALE-WALL C1), but for different mechanisms. `run_ss2.sh`: C1 widened `ST_MAXSEQ` 64->256, so its 97-byte fixture went from silently capped-at-64 to full-97 — not a regression, just a stale frozen baseline. Fixed on `feat/ss2-baseline-rebase` (BASE_REF advanced to `7d497d7c`, cert PASSes, CI wiring included) — **audited independently 2026-09-24 (git-show path, 3 negative controls red) and merged to local master `357574c1`.** `run_ss3_blob.sh`: SEQLEN=24 never hits either `ST_MAXSEQ` cap, so a transport-free RoPE-on/off matched-arm probe was used instead — RoPE OFF reproduces the pre-C1 numbers byte-for-byte and PASSes; RoPE ON (default, current behavior) FAILs with a large margin (worse parent 2.4848 vs merged 2.9047, -0.42 nats). Naive weight-averaging merge (`st_merge_cohort`, from SS-3) no longer satisfies "merged <= worse parent" once RoPE makes two differently-ordered training runs diverge into misaligned loss basins — an emergent architecture interaction (RoPE's own grad-check/KV-equivalence held at `7d497d7c`), not a code bug. **This is a design fork — not decided by this run:** (a) accept the guarantee is gone, loosen/retire the check, (b) make `st_merge_cohort` basin-aware (nontrivial ML change), (c) treat as an operational constraint (only merge peers sharing a recent checkpoint). Escalated to `pkernel-baton.md` judgment-pending; `run_ss3_blob.sh` stays unwired. **2026-09-23, same run — the aarch64 batch checked too:** all 8 built+booted locally (isolated container, `qemu-system-aarch64` TCG-only, no KVM). `run_mc2_slice.sh`/`run_mc2_boot.sh`/`run_mc2_equiv.sh`/`run_smp1.sh` PASS cleanly (incl. their falsifiers) and are wired on `feat/aarch64-mc2-smp-ci-wire` (`continue-on-error: true`, mirroring `run_smp0.sh`'s own precedent) — **audited independently 2026-09-24 (4 scripts × 2 passes, falsifiers checked) and merged to local master `e28d48ed`.** `run_smp2.sh`/`run_smp3.sh`/`run_smp4.sh`/`run_smp5.sh` all FAIL (smp2 checked twice, deterministic both times): every one shows the secondary CPU entering the production dispatcher but the task body never actually running (`ran=0` / `tcb=ok=0` / `H_smp=0x0`) — the same shape as `run_smp0.sh`'s own documented 14/15-red history on GitHub Actions. Left unwired; this looks like the SAME already-escalated secondary-CPU-dispatch-under-TCG gap, not 4 new bugs, but that is a read of the symptoms, not a traced root cause — if a human wants it chased, start from `run_smp0.sh`'s job comment above and this row's evidence. (2026-09-24 audit: smp2's "never entered the PRODUCTION dispatcher" message is misleading — cpu1 does enter it, but concurrent unlocked UART writes garble the line so the grep misses; the real failure is `ran=0`/`ctxtsk(B)=0`.) **2026-09-24: `samples/11_distributed/run_*.sh` (26) + `samples/19_dynamic_id` were never in CI.** One probe run each in pkernel_audit_ss: 3 wired on `feat/distributed-samples-ci-wire` (**audited + merged 2026-09-24, `930d5076`**). `run_crash_recovery.sh` SEGFAULT = **real, old bug** (also at 06-25 `ff5cee8b`): `knl_dispatch_to_schedtsk` (hosted x86_64) enters idle with rsp misaligned by 8 → `movdqa` fault in `knl_idle_wait` on the guard kill path. Fix + CI wiring on `feat/force-dispatch-stack-align` `b34f0e52` — **audited + merged 2026-09-25, `b7b4b5f1`** (master SIGSEGV 3/3 → branch PASS 3/3; gdb: idle entry after the killer rsp%16 0→8; crown unchanged). **D5-k (pre-existing):** `tk_exd_tsk` frees its own stack then force-dispatches without leaving it, so hosted idle runs on freed memory when nothing is ready; upstream ports switch to `knl_tmp_stack` first. Guard path unaffected (static death stack). **linux_x86_64 fixed on `feat/hosted-exd-tmp-stack`, audited + merged 2026-09-25, `9e5a7367`** (cert `tests/host/run_idle_freed_stack.sh` red 2/2 → green 2/2; independent gdb AreaQue walk: 0 free-area idles after the fix; crown unchanged). Open: linux_aarch64 same shape (unfixed); cert needs gdb, not in CI. `feat/distributed-samples-ci-wire-2` (memory_thought + dynamic_id) **audited + merged 2026-09-25, `cd5efa9d`** (3/3 each; 2 new NCs red: wrong-class retrieval votes, relay never evicts; ~24 s gossip margin, PASS under CPU stress). **Audited + merged 2026-09-24:** `feat/so-node-generated-header-deps` (galaxy.lo lacked the generated-header dep; clean tree rc=2→0, `99c17790`), `feat/relay-failover-cleanup-hang` (181 s hang on feeder sleeps → 26–28 s, FAIL path too; merge after `99c17790`). Open: the 3 that failed building `galaxy.lo` now build, but running them (needs qemu-aarch64) is unchecked; 6 are demos with no assertion. Details: baton / branch commit. **D5-j (2026-09-25) root-caused, design fork, unfixed:** `run_cradle_live.sh` cure: every S ingest returns 0 — the boot fixture batch (192 triples, 8 per ~14 s DMN call) holds the `4fdea3d3` freeze 318 s, longer than the arm. Freeze check disabled → green at idle cycle 1, but vacuous: that probe (3.78) came after 8 fixture triples and 0 lesson triples; fixture-only training clears the 5.04 bar. Evidence `pkernel_audit_ss:/build/d5j-0925/`, `d5j2-0925/`. | `ci.yml`, `tests/llm/*.sh`, `tests/aarch64/*.sh` |
...
- **self-access:** R0 read-only SHIPPED (`d730175e`); **R1 first T1 affordance (publish on own topic) shipped+audited 2026-09-21** on `feat/self-access-r1-publish` (343e0d58) — `[self-act-guarded]` matched-arm gate ALL PASS after `net` init (drpc identity); crown `.text` grows +1560B x86 bare-metal / +1440B aarch64 bare-metal (expected, self_access.c/reflex.c are crown-linked). **Independently re-audited and merged to local master 2026-09-21** — crown growth stands; **re-bless approved by mk_pino and recorded 2026-09-24** (docs/audit-trail.md + the ci.yml crown comment; hashes independently re-measured and matched 2026-09-24), push not done. Remaining R1 scope (p-fs write / `mind teach`) deferred: no existing germ-capability table for parent-side code (real design fork, see commit body + judgment-pending). NEXT after re-bless = R2 device-detect loop.
