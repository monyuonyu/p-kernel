...
| D5 | **2026-09-23: 14 more cert scripts found with ZERO CI wiring (a cross-reference of every `tests/**/run_*.sh`/`*_cert.sh` against `ci.yml`, after this same run fixed 6 similar gaps — `[sign-genome]`/`[sign-selflayer-live]`/`[unbounded-dkva]`/L2+mind-pause+dmn-pause/federation-f1/self-access-r1, all now wired).** All 14 are confirmed LIVE, falsifier-backed certs, not superseded/historical (checked headers). **Cheap, do these first (host-only, no special hardware, same pattern as this run's fixes):** `tests/llm/run_mc0.sh`, `run_mc1.sh`, `run_diverse_proof.sh`, `run_ss1.sh`, `run_ss2.sh`, `run_ss3.sh`, `run_ss3_blob.sh`. **Costlier (aarch64, needs `qemu-system-aarch64 -smp N`, some run 3-10x for flake-detection — budget/timeout judgment before wiring):** `tests/aarch64/run_mc2_boot.sh`/`run_mc2_equiv.sh`/`run_mc2_slice.sh` (slice is cheapest, no `-smp` needed), `run_smp1.sh` through `run_smp5.sh` (smp4's "one-mind under SMP" crown cert is probably the highest-value one of the 5). One branch per script (or per small cluster), verify locally before wiring like this run's fixes, same `timeout N | tee log.log` + `grep PASS` shape | `ci.yml`, `tests/llm/*.sh`, `tests/aarch64/*.sh` |
