...
| D5 | **2026-09-23: 14 more cert scripts found with ZERO CI wiring** (a cross-reference of every `tests/**/run_*.sh`/`*_cert.sh` against `ci.yml`, after this same run fixed 6 similar gaps — `[sign-genome]`/`[sign-selflayer-live]`/`[unbounded-dkva]`/L2+mind-pause+dmn-pause/federation-f1/self-access-r1, all now wired). **4 of the 7 cheap host-only ones now wired too** (`run_mc0.sh`/`run_mc1.sh`/`run_diverse_proof.sh`/`run_ss1.sh`, verified locally then merged into `shipped-llm-certs`). **`run_ss2.sh` is a genuine open finding, not wired**: it FAILS today against its own documented pre-SS-2 baseline (`5edbeeda`) — M-tier forward hash `0x2caed71d...`/loss `5.5598` (current) vs `0xb94b1d57...`/loss `5.5406` (base), a real, reproducible mismatch (verified with the correct base student.c+student.h supplied via `SS2_BASE_STUDENT`, ruling out a tooling artifact). Unknown whether this is (a) a genuine M-tier regression introduced somewhere between SS-1 and today, unrelated to SS-2 itself, or (b) the baseline being too stale to mean anything after everything that has landed since SS-1 — **needs investigation before deciding whether to fix code or retire/rebase the cert's baseline**. **Still to verify+wire:** `run_ss3.sh`, `run_ss3_blob.sh` (host-only, cheap, this run started verifying them but ran out of time). **Costlier (aarch64, needs `qemu-system-aarch64 -smp N`, some run 3-10x for flake-detection — budget/timeout judgment before wiring):** `tests/aarch64/run_mc2_boot.sh`/`run_mc2_equiv.sh`/`run_mc2_slice.sh` (slice is cheapest, no `-smp` needed), `run_smp1.sh` through `run_smp5.sh` (smp4's "one-mind under SMP" crown cert is probably the highest-value one of the 5). One branch per script (or small cluster), verify locally before wiring, same `timeout N | tee log.log` + `grep PASS` shape | `ci.yml`, `tests/llm/*.sh`, `tests/aarch64/*.sh` |
