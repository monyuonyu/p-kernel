...
### 7.20 B-STRの実害も実際に再現した（2026-09-17、matched-arm、決定的・非確率的）

§7.7で発見した層Bパッチ`B-STR`（`kernel/mtkernel3/kernel/tstdlib/string.c`の
`knl_memset`、ワード単位書き込みの基準を`sizeof(unsigned long)`に統一した修正）も、
§7.8/§7.10時点では静的アンカーの陰性コントロール確認のみで、「実際にこの差し替えが
起きたら何が壊れるか」は未実証だった。コード自身のコメント（60行目付近）が
「8バイト書いてnを4減らすことになり、要求サイズの約2倍を塗り潰して呼び出し元の
スタックを破壊していた」と明言していたので、これを実測で検証した。

**実験**: ブート不要、QEMUもコンテナも不要——`knl_memset`はカーネル型・グローバル状態に
依存しない純粋な関数なので、`/home/shota/pk-scratch/bstr_harm_probe.c`にFIXED版
（現行master、この`string.c`から関数本体をそのままコピー）とBROKEN版
（`pk-scratch/upstream-neg-root`の上流純正`string.c`から関数本体をそのままコピー、
`n -= 4`固定・`(long)cp % 4`固定、`lp`はどちらの版も`unsigned long*`のまま）を
並べ、両方をこのホスト（x86_64 Linux、`sizeof(unsigned long)==8`と実測確認）上で
ネイティブ`gcc -O0`でコンパイルし同一プロセス内で呼び出す、ソースレベルの
matched-arm比較にした（リポジトリのファイルは一切変更していない、使い捨て）。
256バイトのカナリア（`0xAA`）→32バイトのターゲット領域→256バイトのカナリアという
バッファを用意し、ターゲットに対して`n=32`で`0x5A`を敷き詰めさせ、ターゲット自身の
中身と後続カナリアの両方を検査した。

**結果**（`python3`の`subprocess.run`経由でコンパイル・実行、決定的・毎回同一）:
- **FIXED**: ターゲット32バイト全て`0x5A`、後続カナリア256バイトは`0xAA`のまま
  （0/256バイト破壊）。
- **BROKEN**: 2種類の実害が同時に観測された。
  1. **ターゲット領域内部が不完全**: 32バイト中16バイト（オフセット4-7,12-15,
     20-23,28-31——8バイトワードごとの上位4バイト）が`0x5A`ではなく`0x00`。
     原因は`lval`の生成コード自体が上流のまま32bit分（`c|c<<8|c<<16|c<<24`）
     しか埋めておらず、64bitの`unsigned long`に書き込むと上位32bitが未初期化の
     まま`0`で漏れるため（現行masterはここに`if(sizeof(unsigned long)>4) lval |=
     lval<<16<<16;`を追加して塞いでいる）。
  2. **後続カナリアへの越境書き込み**: 256バイト中32バイトが破壊された
     （`0xAA`→書き込み値）。`n>=4`条件と`unsigned long*`（8バイト）の
     `*lp++`が組み合わさり、n=32に対して8バイト×8回=64バイト書き込まれ、
     要求サイズのちょうど2倍——コード自身のコメントの「約2倍」という記述と
     数値まで一致する。

**この検証がB-INITSTK/B-INITTASK-EXITと違う点**: ブート・タイミング・割込みに
一切依存しない、純粋にソースコードの構造から来る決定的な結果——同じ入力なら
何度実行しても同じ16バイトの欠落と同じ32バイトの越境が起きる（乱数もスケジューラも
関与しない）。したがって反復試行（N=10やN=60のような）による頑健性の議論は
そもそも不要で、1回の実行で十分（実際、内部でFIXED/BROKEN両方を同一プロセス内で
連続実行しても結果は安定していた）。

**この発見の意味**: 破壊される16バイトは「値が違う」だけの実害だが、越境する
32バイトは呼び出し元のスタック/ヒープの隣接領域を書き換える——`knl_memset`は
カーネル内部で構造体初期化・TCB/バッファのゼロクリア等、広く使われる基礎関数
（`grep`では`kernel/mtkernel3/`配下で2桁の呼び出し箇所がある）なので、この
パッチが消えれば影響範囲は特定の一機能ではなく「8バイト以上をmemsetする
すべての呼び出し」になる。B-INITSTK（遅れて別の場所で症状が出る）・
B-INITTASK-EXIT（即座かつ全面的に症状が出る）に続く第三のパターンとして、
「即座に・広範囲に・だが局所的な値の破損として」症状が出るケースと言える。
ゲートの位置づけ（非BLOCKING、CI配線済み、19本中の`B-STR`アンカー）は変わらない。
...
実験用プローブ自体が、SCHED_RR/SCHED_FIFOのどちらとも無関係な、現行masterの
未修正のブート済みツリーそのものに存在する別のハングを踏んだため。この発見は
`docs/architecture/gap-ledger.md`に新規OPEN行`RNG0-BUSY-TASK-STALLS-DISPATCH`
として記録した（詳細・再現手順は同ファイル参照、ここでは要約のみ）。

**経緯**: `/home/shota/pk-scratch/schedrr-baseline`・`-broken`worktree
（`arch/x86/selftest.c`にT17プローブ——同一優先度2つのring0タスクを
SCHED_RRでbusy-loop実行し、両方がCPU時間を得るか確認する設計——を追加、
`-broken`側は`timer.c`のSCHED_RR tick処理ブロックも除去）は、実は
2026-09-16 03:30-03:58のrunが既に作っていた孤児（このrun自身が記録を残さず
死んだため未報告のままだった、`pkernel_audit_ss`にビルド・ブートログも残存）。
これを引き継いで続行したところ、baseline・brokenのどちらも、T17プローブ内の
`tk_dly_tsk(500)`（親タスクが2つの子タスクを起動した直後に500ms待つ箇所）で
**恒久的にハングする**ことが分かった（90秒のハーネスタイムアウトで強制終了、
例外なし）。デバッグprintを挿入して切り分けた結果:
- SCHED_RR・SCHED_FIFOのどちらでもハングは同一（RR固有ではない）。
- 子タスクを2つから1つに減らしてもハングは同一（2タスク間の相互作用ではない）。
- 子タスクのbusy-loop本体を`while(!stop){counter++;}`から
  `while(!stop){counter++; tk_dly_tsk(10);}`（10msごとに自発的にカーネルへ
  戻る）に変えると、**クリーンに3秒で完走**（`tk_dly_tsk(500)`が正常に戻る）。

**切り分けられた条件**: カーネル呼び出しを一切行わないring0タスクが1つでも
動いている間、別の（より高優先度の）タスクの`tk_dly_tsk`によるタイマ待ちが
永遠に明けない、という疑い。4通りの設定（RR・FIFO・単一タスク・FIFO再現N=2）
すべてで100%再現、対照実験（10ms毎に自発的にカーネルに戻る版）では0%——
偶然のばらつきに見える余地がない決定的な分離。

**確認して否定した仮説**: 「新規タスクは割込み禁止のまま開始するのでは」を
`kernel/mtkernel3/kernel/sysdepend/x86_pc/dispatch.S`を読んで検証したところ、
...
### 7.26 RNG0ハングの失敗する回帰テストを作った（2026-09-20、mk_pinoの判断待ち項目8への回答に基づく）
