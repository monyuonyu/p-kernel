...
| `config/config.h` | 新アーキ用 `#ifdef _LINUX_X86_64_` 等の追加ブロック | **対象外と判断**（うるさく消える側。ビルド設定値なので消えれば非対応アーキがビルドできなくなる。裏取り未検証、§5-5参照） |
| `include/sys/inittask.h` | 既定値への `#ifndef` ガード追加 | 同上、対象外 |
| `include/sys/machine.h` | 新アーキ用 `#include "sysdepend/..."` 分岐追加 | 同上、対象外 |
| `include/tk/syscall.h` | `T_DSSY` に `startupfn`/`cleanupfn`/`resblksz` 追加 | 同上、対象外（ただしこれは構造体レイアウト変更なので本当に「うるさい」か要検証） |
...
### 7.6 新発見: SCHED_RR ラウンドロビン・タイムスライスは無防備な層Bパッチだった
