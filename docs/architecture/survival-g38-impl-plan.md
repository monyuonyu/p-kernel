...
**Wave G38.0 — the seam (smallest thing that makes the cert *possible*).** Refactor `moe_select_step` out of `select_expert` (touchpoint 0). Acceptance: `[moe-osc]`/`[moe-twolayer]`/`[moe-nocentral]` byte-identical pre/post. No behaviour change, no new claim. *This wave exists solely to close trap A2 — without it every later cert is a rubber stamp.*
