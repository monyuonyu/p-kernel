...
Status: **R0✅ R1✅ R2✅（配管完了）/ R3 width 拡張も着地（LM-9 Part X surgery: R_DM 32→48）・学習面は前進** / 最終更新: 2026-06-19 doc-status fix（旧: 2026-06-07）
（doc-status fix: 旧「R3 width 拡張は未着手」は STALE。思考の幅 `R_DM` は実際に 32→48 へ拡張済み — `arch/common/r3_incontext.c:99` の `#define R_DM 48`、living-mind.md Part X 参照。）

> **状態 (2026-06-07)**: R0–R2 の配管は完了済。§1 の「現状の正確な姿」はこの再設計**前**のスナップショット
> である点に注意（例：`DNODE_MAX` は R0 で 8→32 に上がり実機検証済、全対全 pub は REGION スコープで部分集合配信に）。
> R3 のうち **width 拡張（思考の幅 `R_DM`）は着地済み**（LM-9 Part X surgery で 32→48、`r3_incontext.c:99`；
> さらに実用サイズへの拡大は段階的に継続）。一方 §7 が要求した
> **学習面は前進**した — R3a オンデバイス逆伝播学習（dtr 26.7→95–98%、重みを p-fs へ・peer がロード）と
...
- `DNODE_MAX = 8` 固定。**8台で頭打ち。**（`arch/common/include/drpc.h`）
  〔状態 2026-06-07：R0 で **`DNODE_MAX = 32`** に引き上げ・実機検証済。32 超は federation.md の本丸〕
