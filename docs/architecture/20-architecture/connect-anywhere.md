...
## SLICE 4 — Automatic relay-transport fallback (UDP↔TCP) — SHIPPED + AUDITED (build detail trimmed 2026-07-01)

> This slice (v1 of the §4 ladder) is SHIPPED, AUDITED, and integrated to master. The runtime
> ladder that drives it is §4 above (kept). The ~230 lines of build/state-machine detail here
> are preserved in git history: `git show 79518a33:docs/architecture/connect-anywhere.md`. The
> crown-safety ground rule (§0) and the honest bound on what stays unreachable (§5) remain live.
