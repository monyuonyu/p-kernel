...
| Conscience floor = HARD invariant #5 (ENFORCED, gate 6) | manifest `invariant_ids[GEN_INV_FLOOR]` carries the predecessor's ACTUAL verified floor-chain head (`law_floor_head_id`); `compat_ota_accept_gen` gate 6 REFUSES a successor whose named floor id ≠ this node's verified floor head, or whose floor fails `law_verify` (`OTA_REJECT_FLOOR`) | A successor lacking/weakening the floor content-addresses to a DIFFERENT head id → an ILLEGAL successor, now a FALSIFIABLE check not an assertion (mk_pino: the floor is immutable). Falsifier `[gen-floor-invariant]`: a mismatched-floor successor MUST be refused; `-DGEN_SKIP_FLOOR` vacates gate 6 → the cert goes RED (cross-audit #1). |
