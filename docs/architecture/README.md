...
> **全体構造は [V-MODEL.md](V-MODEL.md)（設計↔検証の V スパイン・各 doc の level と cert）、
> 全 doc 索引は [INDEX.md](INDEX.md)（漏れなく一度ずつ）、OPEN gap の正本は
> [gap-ledger.md](gap-ledger.md)。** この README は下記の**概念地図**（5レイヤー・7軸の
> 読み筋・依存関係図）。doc は V-level サブディレクトリ（`00-concept/` … `50-evolution/`）に
> 置かれ、本文のリンクは basename の `[[wiki]]` で張る（Obsidian 規約で basename 解決 ⇒
> 移設してもリンクは切れない）。
...
>
> **名前の読み方（dream-tier / artifact-tier 規約）。** 「脳」「心」「魂」「考える器官」
> 「集合意識」は**意図的な北極星（dream-tier）であり、削らない**。だが grand name の
> 隣には常に**今この波で実際に動くもの（artifact-tier）**を併記する —— 例: living-mind の
> 「心」の **artifact today** は *連想想起*（`R_DM`=48・16 key×64 answer・single-token・
...
> **→ 全ドキュメント索引: [[INDEX.md]]。** この README §2 は中核 9 本を 7 軸で描いた
> **概念地図**（読み筋）。`docs/architecture/*.md` を**漏れなく一度ずつ**並べた
> 完全な一覧は [[INDEX.md]] にある（各行に一行の役割＋tier）。
...
これがすべての設計判断の試金石である。p-fs の所在引きは
**全員が同じ計算で導く一貫性ハッシュ**であって中央索引ではない。ゲーティングは
**局所勾配**であって中央ゲートではない。どの器官も、この一行を裏切らない。

#### 「中央なし」の正確な意味（誤読を防ぐ精密化）

外部レビューが、この一行を雑に読むと「coordinator が一切いない」と誤解されうると指摘した。
**正確には、coordinator の役割は *存在しない* のではなく、*固定されていない*。** 言い換える:

> region に集約役（coordinator）は居る。しかしそれは**固定された特権ノードではなく、
> 決定的に委譲される役割**である —— 生きているメンバーの中で**最小 node-id** を全員が
> 同じ計算で選ぶ（`region.c` の deterministic coordinator）。誰も「中央サーバ」を任命せず、
> その役を担うノードが死ねば、次に最小の生存 id が同じ計算で自動的にその役を継ぐ。

そして決定的に重要なのは次の一点である:

> **その coordinator が集約の最中にクラッシュしたときに全体が止まらないことは、
> 「前提」ではなく「証明義務（proof obligation）」である。** 設計上そうなっているはず、では
> 不十分で、**実プロセスを kill してネットワークが集約を完遂し続けることを `[live]` で実証**して
> 初めて、この不変条件は守られたと言える。検証ティアの定義は
> [[gap-ledger.md]] の `[live]`/`[in-proc]` 規約を参照。

この証明義務に対する live cert は別レーンが書く（kill-during-aggregation の走行系テスト）。
本ファイルは**主張を精密化するだけ**であり、cert が green になるまで「coordinator のクラッシュ
耐性は証明済み」とは書かない。関連する既存の `[live]` 実証は survival ループ
（kill 中に全推論完遂・`degraded(k/n)`、`13_survival_loop`★）と G13 の arrival-driven 集約
（never-arriving メンバーで cap が発火）であり、coordinator-crash-during-aggregation の
専用 live cert はその延長線上にある。
...
（下表は中核だけの概念地図。**全ドキュメントの完全索引は → [[INDEX.md]]**。）
