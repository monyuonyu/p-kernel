...
| **LM-13** | graceful forgetting (`mind forget`) — chosen forgetting is WISE, not just old | min-EARNED-salience eviction + `r3_fact_touch` (one accrual site), `r3_incontext.c`; [living-mind-lm13-forgetting.md](living-mind-lm13-forgetting.md) | — | `[forget-baseline]` `[forget-unearned]` `[forget-cured]` `[forget-noregress]` `[forget-onesite]` | The R3 fact-queue's FIFO eviction becomes min-EARNED-salience; `mind ask` accrues salience (clamped to 8). With **zero asks** it degenerates BYTE-IDENTICALLY to FIFO (evicts the oldest fact, seq 1; its trace decays **100%→64.0%**); asking f1 **12×** (clamp 8) flips the eviction to f2 and f1 SURVIVES at **100.0%** — a measured **+36-pt** gap through the SAME arrivals/budgets/eval, the ONLY delta being the asks (the load-bearing falsifier). "LM-1 cured un-chosen forgetting; LM-13 makes chosen forgetting wise." `dmn.c` byte-identical, no wire change. |
| **LM-14** | curiosity (`mind curious` / `mind wonder`) — the mind knows what it does NOT know, and treasures the answer | want-table (DUAL of LM-13 salience) + `r3_want_note` (one accrual site, m_ask MISS branch) + want→salience conversion in `r3_fact_learn`, `r3_incontext.c`; `EV_WONDER=20`; [living-mind-lm14-curiosity.md](living-mind-lm14-curiosity.md) | — | `[curio-prior]` `[curio-accrue]` `[curio-unwondered]` `[curio-wondered]` `[curio-weightknown]` `[curio-clean]` `[curio-bounded]` | The mind REMEMBERS keys it was ASKED about but did NOT know (share-qualified: unbound **AND** masked share < 75), speaks them (`mind wonder`, `EV_WONDER`), and when a wanted key is finally TAUGHT the accrued want CONVERTS into arrival salience. GENERIC BY CONSTRUCTION (the LM-13 lesson): the load-bearing claim is pure **SELECTION** (evicted-`seq`), NO accuracy pathology. Two runs are byte-identical except **12 pre-teach wonders** of k_u: unwondered → evicted-seq **[1,2,3,4,5]** (f5 dies); wondered → f5 arrival salience **8/8**, evicted-seq **[1,2,3,4,6]** (f6 dies, **f5 SURVIVES**). The share qualifier (guarding LM-13 §4's ~100%-but-evicted facts) is load-bearing: dropping it reddens `[curio-weightknown]`. "LM-13 made chosen forgetting wise; LM-14 makes the mind know what it doesn't know." `dmn.c` byte-identical, no wire change. |
