...
## 2026-07-11: run_smp0.sh (SMP-autodetect cert) is RED again — pinpoint diagnosis
On qemu-system-aarch64 10.1, `tests/aarch64/run_smp0.sh` FAILS. Two-agent adversarial diagnosis (impl kept dying on API-disconnect; the AUDITOR reproduced it independently):
- The SMP self-test slice still PASSES (autodetect 2 cpus via GICD_TYPER, SMP-RUN/MUTEX/BOOT PASS, counter==400000). Then after "[BOOT] Starting T-Kernel..." the **knl_main() handoff branches to a bad address 0x3000000** → EL1 instruction abort (ESR 0x86000000, EC 0x21, same-EL); `_vec_el1_sync` (arch/aarch64/cpu_support.S:472) prints "SYNC E=..." then **eret's back to a corrupted ELR and re-faults FOREVER** (handler doesn't halt). Deterministic, NOT timing-dependent, identical at -smp 1. A **plain build (no -DSMP_SELFTEST) boots cleanly to the p-kernel> shell** → the abort is specific to the SMP-selftest→knl_startup_hw()/knl_main() handoff (boot/aarch64/main_mtk3.c:439).
- Suspected **μT-Kernel 3.0-migration regression**: ②.2c shipped byte-identical SMP on μT-Kernel **2.0** (2026-06-22); this red is post-[[moment_2026_07_03_tkernel3_migration]] (core 2.0→3.0). The handoff path lives in the swapped core.
- Second, INDEPENDENT blocker (latent since run_smp0.sh's first commit 78c5d222): the harness's tick assertion greps the UART for **"Initial task started" which is printed ONLY by arch/x86/usermain.c** — aarch64's initial task prints a different banner, so the grep FAILS even on a healthy aarch64 boot. FIX ORDER IS LOAD-BEARING: (1) fix the abort FIRST, THEN (2) replace the phantom sentinel with a string aarch64 actually prints — doing (2) alone would turn a true-RED into a FALSE-GREEN. Also: the sync handler should HALT (`b .`) not eret-loop, to make the failure legible.
- The gic-version=2 pin is a verified no-op on this qemu (default is already GICv2). WIP preserved on `fix/smp-gicv2` (origin backup). This is fable5's proposed **Wave-B** (root-cause the abort; 30-boot statistical cert + wave-45 same-day unfixed-control).

### FIXED 2026-07-11 (master 24b01a62, audit-verified integrate, fable5 Wave-B)
ROOT CAUSE (confirmed by a control experiment — bridge disarmed reproduces the exact ELR=0x3000000): the μT-Kernel 3.0 core migration DISCONNECTED the C scheduler from the asm dispatcher under SMP. On 2.0 the core reached scheduler state via CUR_CTXTSK/CUR_SCHEDTSK macros → `g_smpcpu[me].{ctxtsk,schedtsk}` (the SAME per-CPU storage the asm `.Ldispatch_loop` loads). The 3.0 vanilla core writes the RAW GLOBALS `knl_ctxtsk/knl_schedtsk`, and a new `kernel/mtkernel3/kernel/sysdepend/aarch64_virt/cpu_status.h` won the include redirect and drove dispatch off those globals — so after the self-test, the asm loaded a STALE dead-selftest-TCB from the per-CPU slot and `ret`'d into its garbage saved x30 = 0x3000000 (unmapped virt-flash) → EC=0x21 instruction abort.
FIX = a g_smp_prod-gated production-boot scheduler BRIDGE (all SMP_SELFTEST-gated so the crown default build is byte-identical, md5-verified): `.Ldispatch_loop` syncs per-CPU schedtsk DOWN from global knl_schedtsk and mirrors the chosen task UP into global knl_ctxtsk, armed only at the T-Kernel handoff (main_mtk3.c), CPU0-only. Abort GONE across 40+ boots, -smp 2/4 reach the `p-kernel>` shell. Also: hardened `_vec_el1_sync` to print ONE legible dump then `b .` (no more re-fault spam that hid the fault); and fixed the phantom sentinel ("Initial task started" is x86-only) → "p-kernel>". STILL OPEN (advisory cert, not a regression): run_smp0.sh -smp8 self-test slice STARVES on a 4-core PRoot/TCG host (1.6M BKL increments × 8 vCPU) so cert_green not reached here — ThinkPad-runner validation pending; 7 sibling aarch64 certs still carry the phantom sentinel (follow-up).

### 2026-07-11 follow-ups: sentinel migration + a SECOND μT3.0 regression surfaced
- Sentinel migrated off the phantom "Initial task started" (x86-only) → "p-kernel>" (aarch64 real shell prompt, usermain.c:548) in **4** sibling certs (run_smp1, run_mc2_boot[2 sites]/slice/equiv), each verified by real build+boot, no-false-green proven 2 ways. Landed master 27396441.
- **SECOND μT3.0 regression (still OPEN):** `run_smp2/3/4.sh` FAIL TO BUILD on master — `CUR_CTXTSK undeclared` in `smp_prod.c:74/122`, `smp_async.c:152/194`, `smp_onemind.c:113/154` (the ②.2a production-scheduler + ②.2b async-preempt + ②.2c one-mind SMP cert TUs). The CUR_CTXTSK/CUR_SCHEDTSK per-CPU accessor macros were part of what the 2.0→3.0 migration dropped (same root as the 0x3000000 abort — the SMP asm expected per-CPU macros the 3.0 core no longer provides). run_smp0's fix used a g_smp_prod bridge in asm; smp_prod.c/smp_async.c/smp_onemind.c use CUR_CTXTSK in C and need those macros restored (or the same per-CPU accessor reintroduced) before ②.2a-c certs build again. NEXT SMP wave. These 3 certs were left with the phantom sentinel (they red at `make || fail` anyway → no false-green).
