...
#
# Covers the STATE-FSM half of §6-L2 PLUS three now-shipped sub-items: pausing
# mind_net_task/mind_merge_task while HIBERNATING (feat/survival-l2-mind-pause,
# tested separately by run_mind_pause.sh), beacon-cadence reduction
# ([beacon-cadence] below, world_beacon_interval_ms()), and routed-work
# shedding ([hibernate-shed] below, moe.c's eff_state_penalty/
# moe_hibernate_route_test). See world_survival_l2_test's declaration comment
# in world.h for what is STILL deferred (heavy DMN consolidation pause) and
# why.
#
...
#   [hibernate-shed]  moe.c's eff_state_penalty/moe_hibernate_route_test --
#                     PRODUCTION moe_select_step sheds work OFF a HIBERNATING
#                     node toward ACTIVE peers, same shape as [support-route]
#                     already certifies for STRESSED, no pile-on vs a blind
#                     control.
...
# [beacon-cadence]/[hibernate-shed]/[survival-l2] PASS, all three falsifiers
...
one_arch() {  # $1 = boot dir, $2 = human label, $3 = target arch (aarch64|x86_64)
    ...
    echo "$out" | grep -E '^\[(survival-l2|hibernate-reversible|hibernate-gossip|hibernate-not-death|beacon-cadence|hibernate-shed)'
    if echo "$out" | grep -q '^\[hibernate-reversible\] PASS' \
       && echo "$out" | grep -q '^\[hibernate-gossip\] PASS' \
       && echo "$out" | grep -q '^\[beacon-cadence\] PASS' \
       && echo "$out" | grep -q '^\[hibernate-shed\] PASS' \
       && echo "$out" | grep -q '^\[survival-l2\] PASS'; then
        echo "[$label] CURE PASS ([hibernate-reversible] + [hibernate-gossip] + [beacon-cadence] + [hibernate-shed])"
    else
        echo "[$label] CURE FAIL (expected [hibernate-reversible]/[hibernate-gossip]/[beacon-cadence]/[hibernate-shed]/[survival-l2] PASS)"; FAIL=1
    fi
