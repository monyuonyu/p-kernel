...
# tested separately by run_mind_pause.sh) and routed-work shedding
# ([hibernate-shed] below, moe.c's eff_state_penalty/moe_hibernate_route_test).
# See world_survival_l2_test's declaration comment in world.h for what is STILL
# deferred (beacon-cadence reduction, heavy DMN consolidation pause) and why.
#
...
# [hibernate-shed]/[survival-l2] PASS, both falsifiers correctly RED, and the
...
one_arch() {  # $1 = boot dir, $2 = human label, $3 = target arch (aarch64|x86_64)
    ...
    echo "$out" | grep -E '^\[(survival-l2|hibernate-reversible|hibernate-gossip|hibernate-not-death|hibernate-shed)'
    if echo "$out" | grep -q '^\[hibernate-reversible\] PASS' \
       ...
       && echo "$out" | grep -q '^\[survival-l2\] PASS'; then
        echo "[$label] CURE PASS ([hibernate-reversible] + [hibernate-gossip] + [hibernate-shed])"
    else
        echo "[$label] CURE FAIL (expected [hibernate-reversible]/[hibernate-gossip]/[hibernate-shed]/[survival-l2] PASS)"; FAIL=1
    fi
