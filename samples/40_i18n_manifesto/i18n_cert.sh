...
LANGS=$(curl -s --max-time 5 "127.0.0.1:$PORT/langs")
...
for c in $CODES; do
    H=$(mktemp); B=$(mktemp)
    curl -s -D "$H" -o "$B" --max-time 5 "127.0.0.1:$PORT/manifesto?lang=$c" >/dev/null
...
H=$(mktemp)
BCODE=$(curl -s -D "$H" -o /dev/null -w '%{http_code}' --max-time 5 "127.0.0.1:$PORT/manifesto?lang=zz-NOPE")
...
H=$(mktemp)
curl -s -D "$H" -o /dev/null --max-time 5 -H 'Accept-Language: fr-FR,fr;q=0.9,en;q=0.5' \
